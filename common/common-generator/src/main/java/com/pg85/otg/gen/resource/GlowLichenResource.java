...
public class GlowLichenResource extends BiomeResourceBase
{
	...
	public GlowLichenResource(IBiomeConfig biomeConfig, List<String> args, ILogger logger, IMaterialReader materialReader) throws InvalidConfigException
	{
		...
		minY = readInt(args.get(5), Constants.WORLD_DEPTH, Constants.WORLD_HEIGHT);
		maxY = readInt(args.get(6), Constants.WORLD_DEPTH, Constants.WORLD_HEIGHT);
	...
	public String toString()
	{
		...
		s.append(",").append(minY).append(",").append(maxY).append(",").append(countMin).append(",").append(countMax);
