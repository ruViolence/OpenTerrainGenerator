	...
	public void indexWorldObjectsFolder (String worldName) {
		synchronized (indexingFilesLock) {
			if (worldName != null && !customObjectFilesPerWorld.containsKey (worldName)) {
				...
				File worldObjects = new File (OTG.getEngine ().getOTGRootFolder () + File.separator + PluginStandardValues.PresetsDirectoryName + File.separator + worldName + File.separator + "WorldObjects");
				if (!worldObjects.exists ()) {
					worldObjects = new File (OTG.getEngine ().getOTGRootFolder () + File.separator + PluginStandardValues.PresetsDirectoryName + File.separator + "ALL_WORLDS" + File.separator + "WorldObjects");
				}

				if (worldName != null && worldObjects.exists ()) {
					indexAllCustomObjectFilesInDir (worldObjects,
		...
		if (searchDir.exists ()) {
			if (searchDir.isDirectory ()) {
				for (File fileInDir : searchDir.listFiles ()) {
					if (fileInDir.isDirectory ()) {
						...
						if (fileInDir.getName ().toLowerCase ().endsWith (
								".bo4data")
							|| fileInDir.getName ().toLowerCase ().endsWith (
								".bo4")
							|| fileInDir.getName ().toLowerCase ().endsWith (
								".bo3")
							...
							if (fileInDir.getName ().toLowerCase ().endsWith (
									".bo4data")
								|| !customObjectFiles.containsKey (
									fileInDir.getName ().toLowerCase ().replace (".bo4data", "").replace (".bo4", "").replace (".bo3", "").replace (".bo2", ""))) {
				...
				if (searchDir.getName ().toLowerCase ().endsWith (
						".bo4data")
					|| searchDir.getName ().toLowerCase ().endsWith (
						".bo4")
					|| searchDir.getName ().toLowerCase ().endsWith (
						".bo3")
					...
					if (searchDir.getName ().toLowerCase ().endsWith (".bo4data") || !customObjectFiles.containsKey (searchDir.getName ().toLowerCase ().replace (".bo4", "").replace (".bo3", "").replace (".bo2", ""))) {
						customObjectFiles.put (
							searchDir.getName ().toLowerCase ().replace (".bo4data", "").replace (".bo4", "").replace (".bo3", "").replace (".bo2", ""),
							searchDir);
					} else {
						if (OTG.getPluginConfig ().spawnLog) {
